...
const data: uPlot.AlignedData = [
	...
	Array.from({ length: 100 }, (_, i) => Math.sin(i / 9) * 8 + 10),
	Array.from({ length: 100 }, (_, i) => Math.cos(i / 12) * 7 + 10),
];
	...
	return (
		<DemoShell chrome={chrome}>
			<p className="panel-note">
				<strong>Chart.Legend:</strong> click a series to hide it without rebuilding{" "}
				<code>options.legend</code> or recreating the chart.
			</p>
			<Chart data={data} options={options}>
				<Chart.Legend className="tabs" />
			</Chart>
		</DemoShell>
	);
}
